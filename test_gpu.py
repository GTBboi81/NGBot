# test_gpu.py
#
# GPU/Ollama の疎通を確認する診断スクリプト。
# 本体（main.py / tests/run_eval.py）と異なり、接続先をローカルに固定せず
# 環境変数 OLLAMA_HOST に従う。任意のホストを指定して切り分けできることが
# 診断ツールとしての目的であり、送信するのは固定の自己紹介プロンプトのみで
# 通話データや個人情報は一切含まないため、意図的にこの挙動にしている。
